...
public class smoothFallower : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.tag == "portal")
        {
            if (GetComponent<MeshFilter>().sharedMesh == sphere)
            {
                GetComponent<MeshFilter>().sharedMesh = capsule;
                generalControl.instance.increaseCapsuleCount();
        ...
        if (other.tag == "cylinder")
        {
            ...
            if (GetComponent<MeshFilter>().sharedMesh == capsule)
            {
                generalControl.instance.decreaseCapsuleCount();
                Destroy(gameObject);
            }
